 ...
 * ---
 * Evaluates if the target date is the same as the validation date
 */
component extends="BaseValidator" accessors="true" singleton {
	 ...
	 * Constructor
	 */
	DateEqualsValidator function init(){
		variables.name = "DateEquals";
		return this;
	}
	...
	boolean function validate(
		...
		validationResult.addError(
			validationResult.newError(
				argumentCollection = {
					message        : "The '#arguments.field#' is not the same as the validation date of [#compareValue#]",
					field          : arguments.field,
					validationType : getName(),
					rejectedValue  : ( arguments.targetValue ),
					validationData : arguments.validationData,
					errorMetadata  : { "dateEquals" : compareValue }
				}
			)
		);
